...
> slice, **0.6.0 by the bring-your-own Lottie hatch**, and **0.7.0 by the
> per-theme example animation packs** (all in
> `plans/screencast-cut-motion-primitives.md`). The TTS slice (A) is renumbered
> to 0.8.0, B→0.9.0, C→0.10.0 (see Versioning above). State now:
> `pytest plugins/screencast-cut` green; `verify_render.py` exits 0 on
> `golden-cut`, `golden-cut-mp4`, `golden-icons`, `golden-lottie`, and the new
> `golden-themes`; screencast-cut 0.7.0, remotion-video 0.7.1.
