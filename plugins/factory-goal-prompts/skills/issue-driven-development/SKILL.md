...
3. **Author issues - before any code**: split into PR-able slices per `${CLAUDE_PLUGIN_ROOT}/prompts/02-issue-authoring-goal-prompt.md` and the skeleton in `${CLAUDE_PLUGIN_ROOT}/prompts/github-issue-template.md`. Unit of work = 0.5–3 days, independently green, `main` stays shippable. Every body: why-this-slice, verified `file:line` anchors, "Test plan (write these first)", observable acceptance criteria, out-of-scope, `Blocked by #N`. Program-sized work also gets a tracker issue holding the phase DAG.
