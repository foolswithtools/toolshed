#!/usr/bin/env bash
# Public-hygiene sweep for the pdlc-build plugin.
#
...
# names, client or company references, private repo slugs, local filesystem
# paths, anything owner-identifying beyond the public author block).
#
...
#
# Usage:
#   PDLC_BUILD_HYGIENE_DENYLIST=/path/to/denylist.env \
...
# (i.e. the pdlc-build plugin root when run from scripts/).
#
...
denylist="${PDLC_BUILD_HYGIENE_DENYLIST:-}"
plugin_root=""
...
if [ -z "$denylist" ]; then
  echo "usage: PDLC_BUILD_HYGIENE_DENYLIST=<path> $0 [plugin_root]" >&2
